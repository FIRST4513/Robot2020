...
public class udpSubSys extends Subsystem {
    ...
    public void periodic() {
        ...
        //if (loopCtr == 0) checkForIncommingMessage();
        //updateLidarDistance();
    ...
    public double getLidarDistance()        { return lidarDistance; }
    ...
    private void checkForIncommingMessage(){
        ...
        String testMsg1 = "TURRET target=T bearing-x=-15.84 bearing-y=46.01 base_range=75.29 slope=-0.83871 x-pos=419 y-pos=81 time=1582500753.4632208 latency=001372075080871582";
        String testMsg2 = "TURRET target=F";

        currMsg = testMsg1;     // TODO remove this line;

        //currMsg = getLastMessage(false);  //   true = (Clear buffer after getting msg) false = (do not clear buffer)

        SmartDashboard.putString("Vision MSG Test", currMsg);
        if ((currMsg.equals("")) || (currMsg.equals("No Msg")) || (currMsg.equals("(Not Running)"))) {
            return;
        }
        try {
            parseIncommingMsg(currMsg);
        } catch (Exception e) {
            //System.out.println("Error in parseing UDP Msg");
            // nothing else to do
        }
