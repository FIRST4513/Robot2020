...
loadMotor.setInverted(false);
    ...
    public TurretSwitchPressed checkTurretLimitSwitches() {
        ...
        boolean leftPressed = turretLeftLimitSwitch.get();
        boolean rightPressed = turretRightLimitSwitch.get();
    ...
    public boolean HoodFullyRetracted() {
        return hoodRetractLimitSwitch.get();
    }

    public boolean HoodFullyExtended() {
        return hoodExtendLimitSwitch.get();
    }
    ...
    public void moveHood(double power) {
        if (HoodFullyExtended()) {
            if (power >= 0) {
                stopHood();
            }
        } else if (HoodFullyRetracted()) {
            if (power <= 0) {
                stopHood();
            }
        }
        //Assuming retraction is negative and extension is positive.
        // We want to be able to move away from the limit switch.

        //Limit switches not pressed so give motor power.
        hoodMotor.set(power); 
    }
}
